...
$build = Join-Path ([IO.Path]::GetTempPath()) ('RoomTitles-' + [Guid]::NewGuid().ToString('N'))
...
try {
    ...
    & $compiler /nologo /target:exe /r:System.Core.dll "/r:$build\KakaotalkBot.exe" "/out:$build\RoomTitleTests.exe" (Join-Path $PSScriptRoot 'RoomTitleTests.cs')
    if ($LASTEXITCODE -ne 0) { throw '네임드 테스트 컴파일 실패' }
    & "$build\RoomTitleTests.exe" $build
    if ($LASTEXITCODE -ne 0) { throw '네임드 테스트 실패' }
} finally {
